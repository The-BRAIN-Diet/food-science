---
title: Excitatory–Inhibitory Balance (GABA–Glutamate Regulation)
fm_id: BRS1(FM5)
parent_brs: BRS1
summary: 'Functional control of excitatory–inhibitory tone through GABA–glutamate balance, supporting neural stability, inhibitory control, and resistance to overstimulation.'
mechanisms_covered:
  - id: BRS1(PM6)
    name: GABA–Glutamate Neurotransmission Balance
    href: /docs/biological-targets/brs1/pm/brs1-pm6-gaba-glutamate-neurotransmission-balance
  - id: BRS1(PM7)
    name: GABA Synthesis Capacity
    href: /docs/biological-targets/brs1/pm/brs1-pm7-gaba-synthesis-capacity
  - id: BRS1(PM8)
    name: Glutamate Clearance & Recycling
    href: /docs/biological-targets/brs1/pm/brs1-pm8-glutamate-clearance-and-recycling
  - id: BRS1(PM9)
    name: Excitotoxicity Modulation
    href: /docs/biological-targets/brs1/pm/brs1-pm9-excitotoxicity-modulation
key_constraints:
  - id: BRS1(KC2)
    name: Amino Acid Quality & Competitive Balance
    type: precursor
    href: /docs/biological-targets/brs1/kc/brs1-kc2-amino-acid-quality-and-competitive-balance
intervention_dominance: Diet-Supported
intervention_breakdown: Food-State Leaning
timing_specific: 'Yes'
coverage_timing: Daily
references:
  - '[1] [Edden et al. (2012)](/docs/papers/BRAIN-Diet-References#edden_reduced_2012)'
  - '[2] [Puts et al. (2020)](/docs/papers/BRAIN-Diet-References#puts_reduced_2020)'
  - '[3] [Mamiya et al. (2021)](/docs/papers/BRAIN-Diet-References#mamiya_precision_2021)'
  - '[4] [Zhou and Danbolt (2014)](/docs/papers/BRAIN-Diet-References#zhou_glutamate_2014)'
hide_title: true
---

<p class="brs-pm-thumbnail-wrap">
  <img
    src="/img/biological-targets/brs1-portrait-thumbnail.png"
    alt="BRS1 — Neurotransmitter Regulation"
    class="brs-pm-thumbnail"
  />
</p>

## BRS1(FM5) - Excitatory–Inhibitory Balance (GABA–Glutamate Regulation)

## 1. Definition

Functional control of excitatory–inhibitory tone through GABA–glutamate balance, supporting neural stability, inhibitory control, and resistance to overstimulation.

## 2. Intervention Breakdown

Food-State Leaning

## 3. Functional Role

↑ inhibitory tone support; ↑ GABA synthesis support; ↑ glutamate control; ↑ excitation–inhibition balance

## 4. Mechanistic Basis (Integrated FM Narrative)

Excitatory–inhibitory balance emerges from the coordinated interaction of several primary mechanisms and supporting biological pools.

### 4.1 Core Primary Mechanisms

- [BRS1(PM6) — GABA–Glutamate Neurotransmission Balance](/docs/biological-targets/brs1/pm/brs1-pm6-gaba-glutamate-neurotransmission-balance)
  Balance between inhibitory GABAergic tone and excitatory glutamatergic signalling relevant to attention, reactivity, and inhibitory control.

- [BRS1(PM7) — GABA Synthesis Capacity](/docs/biological-targets/brs1/pm/brs1-pm7-gaba-synthesis-capacity)
  Capacity to convert glutamate into GABA through glutamate decarboxylase-dependent synthesis.

- [BRS1(PM8) — Glutamate Clearance & Recycling](/docs/biological-targets/brs1/pm/brs1-pm8-glutamate-clearance-and-recycling)
  Control of glutamate accumulation through uptake, recycling, and buffering processes that protect against excessive excitatory signalling.

- [BRS1(PM9) — Excitotoxicity Modulation](/docs/biological-targets/brs1/pm/brs1-pm9-excitotoxicity-modulation)
  Modulation of excessive glutamatergic drive and downstream excitotoxic stress relevant to neurostability and cognitive regulation.

### 4.2 Supporting Biological Pools (Key Constraints)

- [BRS1(KC2) — Amino Acid Quality & Competitive Balance](/docs/biological-targets/brs1/kc/brs1-kc2-amino-acid-quality-and-competitive-balance)
  Provides the shared amino-acid quality and balance pool required for indispensable amino-acid completeness, LNAA ratios, and competitive transport context across multiple BRS1 mechanisms.

### 4.3 Integrated Functional Narrative

Together, these PMs operationalise BRS1(FM5) as coordinated excitatory–inhibitory balance regulation.





### 4.4 Functional Failure Modes

Excitatory–inhibitory balance may weaken when amino acid quality & competitive balance declines or when reliance on incomplete protein sources without complementary pairing.

Reliance on incomplete protein sources without complementary pairing may reduce [BRS1(KC2) — Amino Acid Quality & Competitive Balance](/docs/biological-targets/brs1/kc/brs1-kc2-amino-acid-quality-and-competitive-balance). Chronically low indispensable amino-acid coverage across meals may further strain pool availability, lNAA imbalance favouring transport competition away from key precursors, ultra-processed low-protein dietary patterns, while inconsistent protein distribution across the day.

These pressures may impair [BRS1(PM6) — GABA–Glutamate Neurotransmission Balance](/docs/biological-targets/brs1/pm/brs1-pm6-gaba-glutamate-neurotransmission-balance), weaken [BRS1(PM7) — GABA Synthesis Capacity](/docs/biological-targets/brs1/pm/brs1-pm7-gaba-synthesis-capacity), reduce the effectiveness of [BRS1(PM8) — Glutamate Clearance & Recycling](/docs/biological-targets/brs1/pm/brs1-pm8-glutamate-clearance-and-recycling), and compromise [BRS1(PM9) — Excitotoxicity Modulation](/docs/biological-targets/brs1/pm/brs1-pm9-excitotoxicity-modulation). At the FM level, this may shift BRS1(FM5) toward reduced excitatory–inhibitory balance performance.


## 5. Cross BRS Links

- BRS6-PM1 — Glycaemic Stability
- BRS3-PM1 — Inflammatory Tone Regulation
- BRS4-PM1 — Mitochondrial Bioenergetic Support

## 6. References

1. [Edden et al. (2012)](/docs/papers/BRAIN-Diet-References#edden_reduced_2012)
2. [Puts et al. (2020)](/docs/papers/BRAIN-Diet-References#puts_reduced_2020)
3. [Mamiya et al. (2021)](/docs/papers/BRAIN-Diet-References#mamiya_precision_2021)
4. [Zhou and Danbolt (2014)](/docs/papers/BRAIN-Diet-References#zhou_glutamate_2014)
