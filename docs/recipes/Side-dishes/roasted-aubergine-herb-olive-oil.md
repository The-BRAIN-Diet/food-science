---
id: roasted-aubergine-herb-olive-oil
title: Roasted Aubergine with Herb Olive Oil
sidebar_label: Roasted Aubergine with Herb Olive Oil
description: Soft roasted aubergine finished with a late-added herb oil of rosemary, thyme, chilli flakes and olive oil, served with a light balsamic drizzle.
list_image: /img/recipes/Roasted-eggplant.webp
tags:
  - Recipe
  - Side Dish
  - Gut Microbiome
  - Inflammation
  - Oxidative Stress
  - Insulin Response
  - Aubergine
  - Rosemary
  - Extra Virgin Olive Oil
servings: 4
recipe_ingredients:
  - display: 1 large aubergine (~450 g)
    food_slug: aubergine
    quantity: 450
    unit: g
    calculation_weight_g: 450
    included_in_default: true
    preparation_state: raw
    composition_basis: named food-page record (Eggplant, raw; FDC 169228)
    conversion_source: recipe-stated edible grams
  - display: 30 ml (2 tbsp) olive oil, for initial roasting
    food_slug: extra-virgin-olive-oil
    quantity: 30
    unit: ml
    calculation_weight_g: 27
    included_in_default: true
    composition_basis: named food-page record (Oil, olive, salad or cooking; FDC 171413)
    conversion_source: "USDA SR Legacy FDC 171413 food_portion: 1 tablespoon = 13.5 g; 2 tbsp = 27 g"
  - display: 30–45 ml (2–3 tbsp) extra-virgin olive oil, for the herb oil
    food_slug: extra-virgin-olive-oil
    quantity: 37.5
    unit: ml
    calculation_weight_g: 33.8
    included_in_default: true
    composition_basis: named food-page record (Oil, olive, salad or cooking; FDC 171413)
    conversion_source: >-
      Midpoint 2.5 tbsp at USDA SR Legacy FDC 171413 food_portion 1 tablespoon =
      13.5 g. The previous entry recorded 62 g for both oils; the sourced total
      is 60.8 g.
  - display: ~1 g (1 tsp) chopped rosemary leaves
    food_slug: rosemary
    quantity: 1
    unit: tsp
    calculation_weight_g: 1
    included_in_default: true
    preparation_state: raw
    composition_basis: named food-page record (Rosemary, fresh; FDC 173473)
    conversion_source: "USDA SR Legacy FDC 173473 food_portion: 1 tsp = 0.7 g; recipe states ~1 g"
  - display: ~1 g (1 tsp) chopped thyme leaves
    food_slug: null
    quantity: 1
    unit: tsp
    calculation_weight_g: 1
    included_in_default: true
    preparation_state: raw
    composition_ref: thyme-fresh
    composition_basis: >-
      USDA SR Legacy FDC 173470 (Thyme, fresh). The Thyme food page records dried
      thyme, which is roughly three times as concentrated.
    conversion_source: recipe-stated ~1 g of chopped fresh leaves
  - display: pinch chilli flakes
    food_slug: null
    calculation_weight_g: null
    included_in_default: false
    excluded_reason: no stated quantity; a pinch contributes under 1 kcal
  - display: salt (roasting and herb oil)
    food_slug: null
    calculation_weight_g: null
    included_in_default: false
    excluded_reason: added to taste, so it is not part of an exact sodium total
  - display: small drizzle balsamic vinegar, to serve
    food_slug: null
    calculation_weight_g: null
    included_in_default: false
    excluded_reason: no stated quantity
nutrition_assumptions:
  - Sodium excludes salt added to taste.
  - >-
    Aubergine is calculated at its raw weight; roasting removes water but does not
    add or remove nutrients.
---

<img src="/img/recipes/Roasted-eggplant.webp" alt="Roasted aubergine with herb olive oil and balsamic drizzle" width="480" />

## Overview

This aubergine dish is built around a two-stage roasting method: first to soften and caramelise the flesh, then to absorb a late-added herb oil that carries garlic, thyme, rosemary, and chilli. Adding the oil toward the end prevents scorching while allowing the aubergine to soak up flavour and develop a glossy finish.

A small drizzle of balsamic vinegar at serving sharpens the dish and balances the richness of the olive oil.

---

## Ingredients (serves 4)

- 1 large aubergine (~450 g / 1 lb, approx)
- 30 ml (2 tbsp) olive oil (for initial roasting)
- salt

### Herb olive oil
- ~1 g (1 tsp chopped rosemary leaves, approx)
- ~1 g (1 tsp chopped thyme leaves, approx)
- pinch chilli flakes
- pinch salt
- 30–45 ml (2–3 tbsp) extra-virgin olive oil

### To serve
- small drizzle good balsamic vinegar

---

## Method

### 1. Prepare the aubergine
1. Cut the aubergine into large cubes or thick slices.
2. Toss lightly with olive oil and salt.

---

### 2. Start roasting
1. Heat oven to **220 °C (200 °C fan)**.
2. Spread aubergine on a tray in a single layer.
3. Roast **20–25 minutes** until softening and beginning to brown.

---

### 3. Prepare the herb oil
1. Grind rosemary, thyme, chilli flakes and salt together  
   *(use a grinder or mortar)*.
2. Stir the mixture into the olive oil.

---

### 4. Finish the aubergine
1. About **15 minutes before the end**, spoon the herb oil over the aubergine.
2. Toss gently to coat.
3. Return to the oven and roast until soft, caramelised and glossy.

---

### 5. Serve
Transfer to a plate and drizzle lightly with balsamic vinegar just before serving.

---

## Serving Suggestions

Serve alongside roasted meats, poultry, or grain-based dishes.

Pairs especially well with:

- [Roast Duck Breast with Berry Sauce](/docs/recipes/Dinner/roast-duck-breast-berry-sauce)

- [Saffron Roast Potatoes](/docs/recipes/Side-dishes/saffron-roast-potatoes)

---

## Notes

- Aubergine absorbs oil readily; adding the herb oil late prevents it becoming greasy.
- Grinding the herbs releases aroma and helps them disperse evenly.
- Balsamic vinegar adds acidity and contrast without masking the herb flavours.

## Brain Health Notes

- **Aubergine** provides fibre and polyphenols that support gut microbiome diversity and contribute to antioxidant networks.
- **Rosemary and thyme** are rich in polyphenols and terpene compounds that have been studied for antioxidant and anti-inflammatory effects.
- **Extra-virgin olive oil** contributes monounsaturated fats and phenolic compounds associated with cardiometabolic and brain health.

## Nutrition

<RecipeNutrition details={frontMatter} />

## Foods/Substances

<RecipeFoods details={frontMatter} />

## Biological Target Matrix

<RecipeMatrix details={frontMatter} />
