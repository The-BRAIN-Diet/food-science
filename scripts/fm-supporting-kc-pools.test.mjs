...
test("FM KC list is the PM-derived union and does not borrow a neighbouring KC", () => {
  ...
  assert.match(markdown, /\(Key Constraint\) \(KC1\) — One-Carbon Donor Pool/)
  assert.match(markdown, /class="brs-kc-title-link"/)
  assert.doesNotMatch(markdown, /Open KC →/)
  assert.doesNotMatch(markdown, /^-\s+\[BRS2\(KC1\)/m)
