...
**Collapsed — Bioactive compounds.** ALA, EPA or DHA at ≥50 mg per serving, and other defensible quantitative values. Do **not** also show “Total omega-3” when those component acids are already listed, and do not construct an omega-3 total by summing unconfirmed fields. A food-page `nutrition_supplementary_sources` row with a canonical matching substance tag projects into the recipe at the ingredient relationship: defensible values are gram-scaled, while a supported qualitative row remains `Present — quantity not established` and links to the canonical Substance page. Presence-only, range and proxy values are never aggregated into numbers. Formulation notes without a matching canonical substance relationship do not project.
